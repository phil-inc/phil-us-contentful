...
export default async function GenerateDownloadableResourcePages({
	...
	}: {data?: {allContentfulDownloadableResource: {nodes: Node[]}} | undefined} = await graphql(
		allDownloadableResourceQuery,
	);

	data?.allContentfulDownloadableResource.nodes.forEach(({id, slug, heading}: Node) => {
