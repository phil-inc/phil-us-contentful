...
export function attachSolutionCoreInteractions(): () => void {
  ...
  // Shadow MutationObserver so every instance is tracked and disconnected on
  // teardown (10 instances watch .jx2-shot class changes — they leak otherwise).
  function MutationObserver(cb: any) {
    const obs = new _NativeMO(cb);
    _mutObservers.push(obs);
    return obs;
  }

  // Track element-level listeners so cleanup removes them. Needed for the
  // page-global anchor handlers below, which bind to shared header/footer nodes
  // that survive SPA navigation.
  function addEl(el: any, type: string, handler: any, opts?: any) {
    _elListeners.push([el, type, handler, opts]);
  ...
  return () => {
    ...
    _mutObservers.forEach(o => { try { o.disconnect(); } catch {} }); _mutObservers.length = 0;
    ...
    _elListeners.forEach(([el, t, h, o]) => { try { el.removeEventListener(t, h, o); } catch {} }); _elListeners.length = 0;
    attached = false;
  };
}
