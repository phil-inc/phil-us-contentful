...
export const Head: React.FC<HelmetProps> = ({
  ...
  return (
    <SEO title={getTitle(contentfulPage.title, contentfulPage.displayTitle)}>
      {isProduction && domain !== "phil.us" && (
        <link rel="canonical" href={config.slug} />
      )}
...
const CareerTemplate: React.FC<CareerTemplateProps> = ({
  ...
  const listingURL =
    process.env.GATSBY_CAREER_LISTING_URI ?? CAREER_LISTING_URL;
    ...
    (async () => {
      try {
        ...
        if (rawResponse.status === 200) {
          const content = (await rawResponse.json()) as Array<
