...
const EventRegistration: React.FC<EventRegistrationProps> = ({ data }) => {
  const banners = data.allContentfulResource.nodes
    .map((r) => r.banners)
    .flat(1) as TResource[];
  ...
  const object = parseScript(
    data.contentfulEventRegistration.hubsportEmbedForm as { raw: string },
  );
        ...
        <Expanded id={data.contentfulEventRegistration.id} pt={40}>
          ...
          <Container fluid className={classes.container}>
            <Grid>
              <Grid.Col
                span={{ xs: 12, md: "auto" }}
                order={{ base: 2, xs: 2, sm: 2, md: 1, lg: 1, xl: 1 }}
              >
                <Text mb={42} fs={"18px"}>
              ...
              <Grid.Col
                span={{ xs: 12, md: "auto" }}
                order={{ base: 1, xs: 1, sm: 1, md: 2, lg: 2, xl: 2 }}
              >
          ...
          <Container fluid p={0}>
            <SimpleGrid
              cols={{ base: 1, sm: 2, md: 2, lg: 1, xl: 2 }}
              spacing={26}
            >
              {arr.map((attendee) => (
                <Box key={attendee.id}>
